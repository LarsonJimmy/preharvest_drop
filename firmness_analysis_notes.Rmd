...
This is a file that describes the process for analyzing fruit firmness for a preharvest drop study conducted from Aug.-Oct. 2020 on Red Delicious apples. 12 total fruit from the outside 2 trees per plot these fruit had their internal ethylene concentrations and firmness measured, juice from each apple in a treatment by rep combination was collected and had their brix measured using a refractometer. Two firmness measurements were taken on either side of the apple using a digital penetrometer:
...
An average value of the two firmness measurements was first calculated...
...
shapiro.test(x = firmAug28.resid)
```
...
Levene's test was significant, but shapiro-wilkes test was not...
...
ggplot(firm.med, aes(x = date, y = med.firmness, color = treatment)) +
  geom_point() +
  geom_line() +
  geom_text_repel(aes(x = date, y= med.firmness, label = TeX(c(rep("a*", times = 12), "a*", "a*", "b*", "a*", "a*",  "a*", "b*", "ab*", "ab*", "a*", "c*", "b*", "b*", "a*",
